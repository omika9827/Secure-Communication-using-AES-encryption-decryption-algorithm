...
def convert(lst):
    """The function will convert into binary form"""
    new_lst = []
    for i in lst:
...
def encrypt(ptext):
    ...
    print("After Pre-round transformation: ", s1)
    s2 = SubsNibble(SBox, s1)
    print("After Round 1 Substitute nibbles: ", convert(s2))
    s3 = ShiftRows(s2)
    print("After Round 1 Shift rows: ", convert(s3))
    s4 = mixCol(s3)
    print("After Round 1 Mix Column: ", convert(s4))
    s5 = addKey(int_to_state((w[2] << 8) + w[3]), s4)
    print("After Round 1 add key: ", convert(s5))
    s6 = SubsNibble(SBox, s5)
    print("After Round 2 Substitute nibbles: ", convert(s6))
    s7 = ShiftRows(s6)
    print("After Round 2 Shift rows: ",convert(s7))
    s8 = addKey(int_to_state((w[4] << 8) + w[5]), s7)
    print("After Round 2 add keys: ", convert(s8))
    return state_to_int(s8)


plaintext = int(input("Enter plain text: "), 2) #Input plain Text
key = int(input("Enter key: "), 2)  #Input Key
keyExp(key)
ciphertext = encrypt(plaintext)
print("Cipher text: ", bin(ciphertext))
ct = str(bin(ciphertext))
c.send(ct.encode())
k = str(bin(key))
c.send(k.encode())
